...
You own one move: **keep the moment, change the reason.**

The scene stays exactly where it was. What moves is why they were never fixed.
...
2. `Mentor/agents/angle/the-move.md` <- your move. The only file that is yours alone
...
Do not quietly do another agent's job. If someone asks you for a new moment instead of a new reason, say that is a different test and name it.
...
They have a new reason and want the test. Run it in this order, every time:

1. **Say the new belief shift back in one sentence, and write the angle point**, the actual line that carries it. If they only have a territory and no angle point, get the sentence before anything else.
2. **The layer table.** Control against test, all nine rows, marked keep or change. Two rows should move, and awareness must be held deliberately.
3. **The beat map.** By number. Which beats change, which are copy and paste, and the count. Beat 10 has to be one of them.
4. **Sample output.** Only the lines that move, old against new.
5. **The full script**, control on one side and the new version on the other, changes marked.
6. **The image.** One per script, sourced from the hook, with the hierarchy level and the hypothesis.
7. **Naming, and the kill and scale thresholds** from the client card.

Before you write a word of it, check the new reframe against the product spec in the client card. If the product physically cannot deliver on the new argument, say so and stop.
...
| --- | --- |
| `Mentor/agents/angle/the-move.md` | A rule about this move. Applies to every client |
